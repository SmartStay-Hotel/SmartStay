...
@section('content')
    ...
    <div class="card"
        ...
        <table class="table table-sm table-hover text-center" id="serviceTable">
            ...
            @endif
            {!! Form::model($event, ['route' => ['event.update', $event->id], 'method'=>'PATCH', null]) !!}
            @include('services.event.partial.form')
            {!! Form::close() !!}
        </table>
    </div>
@endsection
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#changeevent').change(function () {
                if ($(this).is(':checked')) {
                    this.checked = (confirm("Are you sure?")) ? $('#selectEventType').prop('disabled', false) : false;
                }else{$('#selectGuest').prop('disabled', true)}
            });
        });
    </script>
